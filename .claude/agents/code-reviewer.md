...
6. **Design adherence** — matches `docs/DESIGN_GUIDE.md` and `BRAND.md` (this repo holds the **canonical** copy — consuming products install the brand from here, so a change to it is a change to every product)? Tokens used — **no raw hex, no ad-hoc spacing**? A11y floor met (focus states, contrast, semantic HTML, alt text, reduced-motion)? Status never color-only?
